class PersonUploader
  ...
  MANDATORY_KEYS = %w[reference email home_phone_number mobile_phone_number firstname lastname address]
  ...
  def upload_persons_hash(person_hashes)
    ...
    old_references = Person.where(reference: person_hashes.map { |s| s[:reference] }).pluck(:reference)
    ...
    Person.create(is_old_group[false])
    ...
    olds = Person.where(reference: old_hash_by_refs.keys).include_attributes_archives
    olds.each do |old|
